...
describe('hubConnection', function () {
    eachTransportAndProtocol(function (transportType, protocol) {
        describe(protocol + ' over ' + signalR.TransportType[transportType] + ' transport', function () {
            it('can invoke server method and receive result', function (done) {
                var message = '你好，世界！';
                var hubConnection = new signalR.HubConnection(TESTHUBENDPOINT_URL, {
                    transport: transportType,
                    protocol: protocol,
                    logger: signalR.LogLevel.Trace
                });
            ...
            it('closed with error if hub cannot be created', function (done) {
                var errorRegex = {
                    WebSockets: '1011|1005',
                    LongPolling: 'Internal Server Error',
                    ServerSentEvents: 'Error occurred'
                };
                var hubConnection = new signalR.HubConnection('http://' + document.location.host + '/uncreatable', {
                    transport: transportType,
                    protocol: protocol,
                    logger: signalR.LogLevel.Trace
                });
        ...
        describe(' over ' + signalR.TransportType[transportType] + ' transport', function () {
            it('can connect to hub with authorization', function (done) {
                return __awaiter(this, void 0, void 0, function* () {
                    var message = '你好，世界！';
                    var hubConnection;
                    let jwtToken = getJwtToken('http://' + document.location.host + '/generateJwtToken')
                        .then(jwtToken => {
                        hubConnection = new signalR.HubConnection('/authorizedhub', {
                            transport: transportType,
                            logger: signalR.LogLevel.Trace,
...
//# sourceMappingURL=signalr-functional-tests.js.map